...
import { createTheme, ThemeProvider } from '@mui/material/styles';
import { useNavigate } from 'react-router-dom';
import { checkEmailExists } from './services'; // קריאת POST
import appsflyerLogo from '../images/appsflyerLogo.png';
import logoapp from '../images/logoapp.png';
...
const EmailCheckPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { exists } = await checkEmailExists(email); // קריאת POST לשרת לאימות

      if (exists) {
        navigate('/table'); // ננווט לעמוד הטבלה אחרי שהמשתמש אומת
      } else {
        setErrorMessage('Email not found');
      }
    } catch (error) {
